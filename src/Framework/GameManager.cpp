...
void GameManager::RegisterKeyDown(KeyCode keyCode) {
  if (keyCode != KeyCode::NONE && m_pressedKeys.find(keyCode) == m_pressedKeys.end()) {
    m_pressedKeys[keyCode] = true;
  }
}

void GameManager::RegisterKeyUp(KeyCode keyCode) {
...
void GameManager::ShowZombiesWon() const {
  ...
  glPushAttrib(GL_CURRENT_BIT);
  glColor3f(1.0f, 1.0f, 1.0f);
  std::string waves = "You survived ["+std::to_string(WaveManager::GetInstance().GetCurrentWave())+"] waves! Press Enter to restart.";
  displayText(NormalizeCoord(WINDOW_WIDTH / 2, WINDOW_WIDTH), NormalizeCoord(50, WINDOW_HEIGHT), 0, 
      waves.c_str(), true, GLUT_BITMAP_HELVETICA_18);
  glPopAttrib();
